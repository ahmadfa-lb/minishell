...
/*   Updated: 2024/09/04 18:02:53 by afarachi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
// 			*result = ft_strjoin(*result, env_value);
// 		else
// 			*result = ft_strjoin(*result, ""); // No env variable found, don't add anything
// 		free(sub_env);
// 	}
// 	else
// 	{
// 		*result = ft_strjoin(*result, "$"); // If start == end, add the literal $
// 	}
// }
...
char *handle_dollar_sign(char *input, t_env *env, int exit_status)
{
    ...
    while (i < ft_strlen1(input) && input[i])
    {
        if (input[i] == '$')
        {
            ...
            {
                // Handle $"" by adding a space to the result
                result = ft_strjoin(result, " ");
                i += 3;  // Move index past $ and the two quotes
            }
            else if (!input[i + 1])  // If $ is at the end of the string
            {
                result = ft_strjoin(result, " ");
                i++;
            }
            ...
            else if (input[i + 1] == '$')
            {
                ...
                i++;  // Move index past the second $
            }
            else if (ft_isdigit(input[i + 1]))
            {
                i += 2;  // Skip the digit after $
            }
            else
            {
        ...
        {
            // Copy the current character to tmp and append to result
...
//         if (input[i] == '$')
//         {
//             if (input[i + 1] == '"' && input[i + 2] == '"' && input[i + 3])  // Check for $ followed by ""
//             {
//                 result = ft_strjoin(result, "");  // Convert $"" to a space
//                 i += 3;  // Move index past the $ and the two quotes
//             }
//             else if (input[i + 1] == '?')
//             {
//                 handle_dollarexitstatus(&result, exit_status, &i);
...
t_list_tokens *dollar_expansion(t_data *data)
{
    ...
    while (current_token)
    {
        ...
        if (current_token->value && ft_strchr(current_token->value, '$') && current_token->quote_type != SINGLE_QUOTE)
        {
            ...
            // Handle splitting only if the token is not quoted
            if (current_token->quote_type == NO_QUOTE)
            {
                split_strings = ft_split(result, ' ');
                if (!split_strings || !*split_strings)
                ...
                {
                    // Single result, just update the token
                    free(current_token->value);
                    current_token->value = split_strings[0];
                }
                else
                {
                    // Multiple results, create new tokens
                    free(current_token->value);
                    current_token->value = ft_strdup(split_strings[0]);
                    array_length = get_string_array_length(split_strings);
                    i = 1;
